...
Multiple bridged TMs can be attached to the same project. Each will show up as a separate row in the Translation Memory list, named e.g. *Supervertaler TM: Acme (PROJ-001)*.
...
- **Per-hit TM attribution.** With several bridges attached, each match's origin strip identifies the specific TM it came from – *Supervertaler: Acme (PROJ-001)* vs *Supervertaler: PATENTS* – so you can tell at a glance which memory contributed which hit.
