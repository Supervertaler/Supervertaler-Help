...
**A large bank sends only what the job needs.** Once your banks together pass about 8,000 tokens, each job gets a selection instead of everything: only the terminology rows whose term occurs in the document (in either column, whichever way the table was written), and – if it is still too big – the notes the model judges relevant after reading the start of the document, asked once per job. The brief and the style rules always go. It happens by itself; there is nothing to set. The **Bank** row's tooltip says when it is happening and names a file under `D:\Supervertaler\memoqank-extracts\` listing what was sent and what was left out, which is the place to look if a translation seems to have missed something you wrote down.
