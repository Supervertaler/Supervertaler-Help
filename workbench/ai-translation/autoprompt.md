...
- **AutoPrompt does NOT use loaded Image Context.** Even when you have figure images loaded via Section 4 of the Prompt Manager, AutoPrompt sends a text-only meta-prompt to the LLM. The source-aware pre-generation passes count figure *references* in the text (`Figure 1`, `Fig. 2A`, …) and feed that count into the analysis summary, but the actual PNG / JPG files are never sent. If your drawings contain critical visual context that should shape terminology decisions (labelled parts, captions, etc.), the generated prompt will not benefit from them. Image Context is consumed at *translation* time, not at AutoPrompt time — see [Image Extractor](../tools/image-extractor.md) for how the images flow into per-segment translations.
