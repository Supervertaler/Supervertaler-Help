...
For **cost**, an `actual` record's figure is the exact token count × the per-model rate from `pricing.json`. Two things can make it differ slightly from your provider's bill:
...
* The cost currently applies the **full input rate to cached tokens** as well, so on cache-heavy runs (a large batch reusing one big system prompt) the cost can be **slightly over-stated**. The token counts remain exact.
